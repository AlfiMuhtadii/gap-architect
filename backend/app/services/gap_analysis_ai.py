    ...
    *,
    ...
    try:
        ...
        if parsed is None:
            ...
            if parsed is None:
                await _log_llm_run(
                    ...
                    response_json={"error": error or "invalid JSON after repair", "fallback_to": fallback_to or "repair"},
        ...
        try:
            ...
            if getattr(provider, "name", "") == "local_llm":
                ...
                try:
                    ...
                    await _log_llm_run(
                        ...
                        response_json=parsed if isinstance(parsed, dict) else {"raw": parsed, "fallback_to": fallback_to},
                        status=LlmRunStatus.FAILED,
            ...
            if parsed2 is None:
                await _log_llm_run(
                    ...
                    model=provider.model,
                    request_hash=_hash_prompt(repaired_prompt),
                    response_json={"error": error2 or "invalid JSON after repair", "fallback_to": fallback_to or "repair"},
                    status=LlmRunStatus.FAILED,
            ...
            try:
                ...
                await _log_llm_run(
                    ...
                    model=provider.model,
                    request_hash=_hash_prompt(repaired_prompt),
                    response_json=parsed2 if isinstance(parsed2, dict) else {"raw": parsed2, "fallback_to": fallback_to},
                    status=LlmRunStatus.FAILED,
        ...
        await _log_llm_run(
            ...
            model=provider.model,
            request_hash=_hash_prompt(prompt),
            response_json={"error": str(exc) or "llm_failed", "fallback_to": fallback_to},
            status=LlmRunStatus.FAILED,
